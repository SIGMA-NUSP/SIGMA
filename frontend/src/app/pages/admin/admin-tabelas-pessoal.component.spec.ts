...
/**
 * Testes de RENDER das TRÊS listagens server-side de pessoal (operadores,
...
describe('AdminTabelasPessoalComponent — canal de erro das 3 listagens', () => {
  ...
  async function configurar(master: boolean): Promise<void> {
    ...
    await TestBed.configureTestingModule({
      imports: [AdminTabelasPessoalComponent],
      providers: [
  ...
  async function renderizar(): Promise<ComponentFixture<AdminTabelasPessoalComponent>> {
    const fixture = TestBed.createComponent(AdminTabelasPessoalComponent);
    await estabilizar(fixture);
    return fixture;
  }

  /** O template tem `ngModel` (buscas) → o DOM só é confiável depois do `whenStable`. */
  async function estabilizar(fixture: ComponentFixture<AdminTabelasPessoalComponent>): Promise<void> {
  ...
  const tabelas = (f: ComponentFixture<AdminTabelasPessoalComponent>) =>
    f.debugElement.queryAll(By.css('table.data-table'));
  const tabOp = (f: ComponentFixture<AdminTabelasPessoalComponent>) => tabelas(f)[0];
  const tabTec = (f: ComponentFixture<AdminTabelasPessoalComponent>) => tabelas(f)[1];
  const tabAdm = (f: ComponentFixture<AdminTabelasPessoalComponent>) => tabelas(f)[2];
  ...
  describe('controles da tela', () => {
    it('com as 3 listagens em erro, as buscas e os relatórios continuam na tela', async () => {
      respostas[EP_OP] = falha();
      respostas[EP_TEC] = falha();
      respostas[EP_ADM] = falha();
      const fixture = await renderizar();
